...
class Monster(object):
    ...
    def draw(self, win):
        ...
        pygame.draw.rect(win, (0, 128, 0), (self.x + 40, self.y , 40, 8))
        # blood that had been used
        pygame.draw.rect(win, (255, 0, 0),(self.x + 40 + self.blood/200*40, 
                                           self.y , 40 - 40*self.blood/200, 8))
...
def fight():
    ...
    while run:
        ...
        # generate monster
        ismonster = True
        while monster_index < 20 and ismonster:
            if len(monsters) < 3:
                monster = Monster(random.randint(300,500), 414, 64, 64, mimg)
        ...
        for pl in plbullets:
            for monster in monsters:
                if collision_check(pl, monster) or collision_check(monster, pl):
                    monster.blood -= player.attack
                    if monster.blood < 0:
                        monsters.pop(monsters.index(monster))
